...
/* Xilinx. We do this to save a few 10s of code space that would be taken up
...
%{mv4fxtuning: --v4fxtuning}"               /* Xilinx: v4fx tuning support */
...
%{static:-dn -Bstatic} \
%{shared:-G -dy -z text} \
%{symbolic:-Bsymbolic -G -dy -z text} -N"                               /* Xilinx */
...
%{symbolic:-Bsymbolic -G -dy -z text %(link_path) } -N"                 /* Xilinx */
#endif
...
#define	CPP_OS_DEFAULT_SPEC "%{mfpu=*: -DHAVE_XFPU}"
...
/* Xilinx */
#define	STARTFILE_DEFAULT_SPEC "ecrti.o%s %{pg: %{!mno-clearbss: xil-pgcrt0.o%s} %{mno-clearbss: xil-sim-pgcrt0.o%s}} \
                                          %{!pg: %{!mno-clearbss: xil-crt0.o%s} %{mno-clearbss: xil-sim-crt0.o%s}} crtbegin.o%s"
...
/* Xilinx 
   We want profile_after_prologue */

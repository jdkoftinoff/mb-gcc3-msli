...
/* Xilinx APU FPU Support */

/* value of -mfpu set by user   */
const char *fpu_type_string;              

/* Enumeration of fpu type      */ 
enum fpu_type_t fpu_type;         
...
char *rs6000_no_clearbss = "default";   /* Xilinx */
...
rs6000_override_options (const char *default_cpu)
{
  ...
  static struct ptt
    {
	 ...
	 {"405", PROCESSOR_PPC405, POWERPC_BASE_MASK},                                          /* Xilinx */
  ...
  /* Look for a value given to the -mfpu switch */
  static struct fpu_type_s {
      const char *name;
      int value;
  } fpu_config_list[] = {
      { "none",    FPU_NONE },
  ...
  struct fpu_type_s *cfg_p;

  /* End Xilinx */

  set_masks = POWER_MASKS | POWERPC_MASKS;                                       /* Xilinx */
  ...
  set_masks &= ~(target_flags_explicit
		 & (MASK_MULTIPLE | MASK_STRING));                              /* Xilinx */
  ...
  if (fpu_type_string != NULL) {
    for (cfg_p = &fpu_config_list[0]; cfg_p->name; cfg_p++) {
        if (!strcmp(fpu_type_string, cfg_p->name)) 
            break;
    }
    
    if (!cfg_p->name) {
        error("Invalid value (%s) given for switch -mfpu=", fpu_type_string);
    } else {
        fpu_type = cfg_p->value;
    }
  }
  ...
  if (!strcmp (rs6000_no_clearbss, "yes")) 
...
legitimate_lo_sum_address_p (enum machine_mode mode, rtx x, int strict)
{
  ...
  if (TARGET_ELF || TARGET_MACHO)
	...
	return false;
      if (GET_MODE_BITSIZE (mode) > 32
	  && !(TARGET_HARD_FLOAT && TARGET_FPRS && ((mode == DFmode) && TARGET_FPU_DF)))                /* Xilinx */
	return false;
  ...
  if (GET_CODE (x) == PLUS 
    ...
    { 
      ...
      return gen_rtx_PLUS (Pmode, sum, GEN_INT (low_int));
	   ...
	   && ((TARGET_HARD_FLOAT && TARGET_FPRS && (mode == DFmode ? TARGET_FPU_DF : 1))           /* Xilinx */
	       || TARGET_POWERPC64
		      ...
		      tree type, int named)
{
  ...
  if (TARGET_ALTIVEC_ABI && ALTIVEC_VECTOR_MODE (mode))
  ...
  else if (DEFAULT_ABI == ABI_V4)
    {
	  ...
	  && (mode == SFmode || ((mode == DFmode) && TARGET_FPU_DF)))                           /* Xilinx */
	{
	      ...
	      tree type, int named)
{
  ...
  if (mode == VOIDmode)
	  ...
	  && (cum->prototype || TARGET_NO_PROTOTYPE))
	{
	  ...
	  else if (TARGET_HARD_FLOAT && TARGET_FPRS && (mode == DFmode ? TARGET_FPU_DF : 1))                /* Xilinx */
		...
		tree type, int *pretend_size ATTRIBUTE_UNUSED, int no_rtl)
{
  ...
  if (DEFAULT_ABI == ABI_V4
      ...
      && next_cum.fregno <= FP_ARG_V4_MAX_REG)
    {
      ...
      int off = (GP_ARG_NUM_REG * reg_size) + ((fregno - FP_ARG_MIN_REG) * UNITS_PER_FP_WORD);          /* Xilinx */
	...
	{
          mem = gen_rtx_MEM ((TARGET_FPU_DF ? DFmode : SFmode), plus_constant (save_area, off));        /* Xilinx */
          set_mem_alias_set (mem, set);
	  emit_move_insn (mem, gen_rtx_REG ((TARGET_FPU_DF ? DFmode : SFmode), fregno));                /* Xilinx */
	  fregno++;
	  off += UNITS_PER_FP_WORD;                                                                     /* Xilinx */
	}
      ...
      || (!TARGET_ALTIVEC_ABI && ALTIVEC_VECTOR_MODE (TYPE_MODE (type))))
    {
	 ...
	 reference.  */
      ...
      rsize = 1;
	   ...
	   && (TYPE_MODE (type) == SFmode || (TYPE_MODE (type) == DFmode && TARGET_FPU_DF)))                /* Xilinx */
    {
      /* FP args go in FP registers, if present.  */
      indirect_p = 0;
      reg = fpr;
      n_reg = 1;
      sav_ofs = (TARGET_FPU_DF ? 8 : 4) * 4;                                                                /* Xilinx. !! Warning */
      sav_scale = (TARGET_FPU_DF ? 8 : 4) * 4;                                                              /* Xilinx. !! Warning */
...
rs6000_split_multireg_move (rtx dst, rtx src)
{
  ...
  if (FP_REGNO_P (reg))
    reg_mode = (TARGET_FPU_DF ? DFmode : SFmode);           /* Xilinx */
  ...
  if (saving_FPRs_inline)
	...
	if ((regs_ever_live[info->first_fp_reg_save+i] 
	     && ! call_used_regs[info->first_fp_reg_save+i]))
            emit_frame_save (frame_reg_rtx, frame_ptr_rtx, (TARGET_FPU_DF ? DFmode : SFmode),                 /* Xilinx */
	...
	{
	  rtx addr, reg, mem;
	  reg = gen_rtx_REG ((TARGET_FPU_DF ? DFmode : SFmode), info->first_fp_reg_save + i);                   /* Xilinx */
	  addr = gen_rtx_PLUS (Pmode, frame_reg_rtx,
			       GEN_INT (info->fp_save_offset 
					+ sp_offset + 8*i));
          mem = gen_rtx_MEM ((TARGET_FPU_DF ? DFmode : SFmode), addr);                                          /* Xilinx */
	  set_mem_alias_set (mem, rs6000_sr_alias_set);
...
rs6000_emit_epilogue (int sibcall)
{
  ...
  if (restoring_FPRs_inline)
	   ...
	   && ! call_used_regs[info->first_fp_reg_save+i]))
	{
	  ...
	  emit_move_insn (gen_rtx_REG ((TARGET_FPU_DF ? DFmode : SFmode),                                   /* Xilinx */
				       info->first_fp_reg_save + i),
			  mem);
	}
	  ...
	  for (i = 0; i < 64 - info->first_fp_reg_save; i++)
	    {
	      ...
	      RTVEC_ELT (p, i+3) = 
		gen_rtx_SET (VOIDmode,
			     gen_rtx_REG ((TARGET_FPU_DF ? DFmode : SFmode), info->first_fp_reg_save + i),              /* Xilinx */
			     mem);
	    }
	}
...
/* Xilinx
 * NO_PROFILE_COUNTERS has to be respected in output_function_profiler 
 */
#ifndef NO_PROFILE_COUNTERS
# define NO_PROFILE_COUNTERS 1
  ...
  switch (DEFAULT_ABI)
      ...
      if (!TARGET_32BIT)
	{
      ...
      ASM_GENERATE_INTERNAL_LABEL (buf, "LP", labelno);
      /* Xilinx
         Removing code to get from lr and store it to the stack.
         Instead, we will let _mcount use the backchain to locate the lr and restore it.
         fprintf (file, "\tmflr %s\n", reg_names[0]); 
      */
      if (flag_pic == 1)
	{
...
rs6000_adjust_priority (rtx insn ATTRIBUTE_UNUSED, int priority)
{
  ...
  case CPU_PPC750:
    ...
    break;
    /* Xilinx. Also enabled #if 1 above */
    case CPU_PPC405:
    {
      ...
      if (recog_memoized (insn) < 0)
        break;
      switch (get_attr_fp_type (insn)) {
        default:
          break;
          
        case FP_TYPE_FP_MADDSUB_S:
          priority <<= 2;
          break;

        case FP_TYPE_FP_ADDSUB_S:
          priority <<= 2;
          break;

        case FP_TYPE_FP_MUL_S:
          priority <<= 2;
          break;
      }
...
rs6000_use_sched_lookahead (void)
{
  if (rs6000_cpu_attr == CPU_PPC8540)
    return 4;
/*  else if (fpu_type != FPU_NONE)                /* Xilinx */ 
/*    return 15; */
...
rs6000_complex_function_value (enum machine_mode mode)
{
  ...
  if (FLOAT_MODE_P (mode) && TARGET_HARD_FLOAT && TARGET_FPRS && (mode == DFmode ? TARGET_FPU_DF : 1))          /* Xilinx */
    regno = FP_ARG_RETURN;
  else
    {
      regno = GP_ARG_RETURN;
...
rs6000_function_value (tree valtype, tree func ATTRIBUTE_UNUSED)
{
  ...
  if (SCALAR_FLOAT_TYPE_P (valtype) && (TARGET_HARD_FLOAT && TARGET_FPRS && (mode == DFmode ? TARGET_FPU_DF : 1)))       /* Xilinx */
    regno = FP_ARG_RETURN;

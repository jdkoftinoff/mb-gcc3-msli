...
void xmd_load_program(char* args, int from_tty);
...
#endif
  long what_they_get = MAX_REMOTE_PACKET_SIZE;
#if 0
  if (config->fixed_p)
    {
      if (config->size <= 0)
...
/* Added by JW to mirror what happens in more recent gdb-6.4 */
  reopen_exec_file ();
  reread_symbols ();
...
// NAVANEE - changes to program loading - send info to XMD to reset target (based on user option) before every program download

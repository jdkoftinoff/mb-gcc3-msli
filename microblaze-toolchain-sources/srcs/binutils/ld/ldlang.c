...
// sathya debug :
asection *bss_heap = NULL;
asection *bss_stack = NULL;
// end
     ...
     boolean relax;
{
  ...
  //  bss_heap = bfd_get_section_by_name (output_bfd, "bss_heap");
  //  bss_stack = bfd_get_section_by_name (output_bfd, "bss_stack");
	...
	case lang_output_section_statement_enum:
	  {
		...
		/* 
		 * Sathya = Fix CR 205522
		 * The following line resets _raw_size of output section
		 * This is needed else the raw size increases based on the
		 * number of relax passes (which in case of MB is 2 and hence
		 * new stack or heap sections have twice the size)
		 */
